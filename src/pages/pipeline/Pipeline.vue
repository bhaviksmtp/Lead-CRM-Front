<template>
  ...
  <div v-else class="pipeline-container">
    ...
    <div class="pipeline-toolbar">
      <!-- Stage Filter/Jump Pills -->
      <div class="pipeline-stage-pills">
        <button
          v-for="col in pipeline"
          :key="col.stage.id"
          class="stage-pill-btn"
          :class="{ active: activeStageId === col.stage.id }"
          @click="scrollToStage(col.stage.id)"
        >
    ...
    <div class="pipeline-board-wrapper" ref="boardWrapperRef">
      <div class="pipeline-board">
        <div
          v-for="col in pipeline"
          :key="col.stage.id"
          :id="`stage-col-${col.stage.id}`"
          :class="[
            'pipeline-column',
            { 'drag-over': dragOverStageId === col.stage.id }
          ]"
          @dragover.prevent="onDragOver(col.stage.id)"
          @dragleave="onDragLeave(col.stage.id)"
          ...
          <div class="column-body">
            ...
            >
              ...
              <div class="kanban-card-footer">
                ...
                <select
                  class="kanban-card-move-select"
                  :value="col.stage.id"
                  @change="moveLeadViaSelect(lead, $event.target.value)"
                  title="Move to stage"
                >
                  <option
                    v-for="s in pipeline"
...
const loadPipeline = async () => {
  try {
    ...
    if (pipeline.value.length && !activeStageId.value) {
      activeStageId.value = pipeline.value[0].stage.id;
    }
    loading.value = false;
  } catch (err) {
...
const scrollToStage = (stageId) => {
  ...
  const colElement = document.getElementById(`stage-col-${stageId}`);
  if (colElement && boardWrapperRef.value) {
    const wrapper = boardWrapperRef.value;
    const leftOffset = colElement.offsetLeft - 16;
